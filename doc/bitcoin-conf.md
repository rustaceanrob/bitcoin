...
The configuration file is used by `bitcoind` and `bitcoin-cli`.
...
Changes to the configuration file while `bitcoind` is running only take effect after restarting.
