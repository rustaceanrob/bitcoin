...
class RawTransactionsTest(BitcoinTestFramework):
    ...
    def getrawtransaction_tests(self):
        ...
        txId = tx['txid']
        err_msg = (
            "No such mempool transaction. Provide a block hash to look it up in a"
            " specific block. Use gettransaction for wallet transactions."
        )

        for n in [0, 2]:
            self.log.info("Test getrawtransaction without blockhash for a confirmed tx raises")
            for verbose in [None, 0, False, 1, True]:
                assert_raises_rpc_error(-5, err_msg, self.nodes[n].getrawtransaction, txId, verbose)

            # invalid parameters - supply txid and invalid boolean values (strings) for verbose
            ...
            # invalid parameters - supply txid and empty array
            assert_raises_rpc_error(-3, "not of expected type number", self.nodes[n].getrawtransaction, txId, [])

            # invalid parameters - supply txid and empty dict
            assert_raises_rpc_error(-3, "not of expected type number", self.nodes[n].getrawtransaction, txId, {})
        ...
        for n in [0, 2]:
            self.log.info("Test getrawtransaction with blockhash")
            # We should be able to get the raw transaction by providing the correct block
            gottx = self.nodes[n].getrawtransaction(txid=tx, verbose=True, blockhash=block1)
            assert_equal(gottx['txid'], tx)
            assert_equal(gottx['in_active_chain'], True)
            self.log.info("Test getrawtransaction without blockhash: expect the call to raise")
    ...
    def getrawtransaction_verbosity_tests(self):
        ...
        for n, v in product([0, 2], [1, 2]):
            self.log.info(f"Test getrawtransaction_verbosity {v} with blockhash")
