...
bool HintfileWriter::WriteNextUnspents(const std::vector<uint64_t>& unspent_offsets, const uint32_t& height)
{
    // First write the current file position for the current height in the header section.
    uint64_t curr_pos = m_file.size();
    uint64_t cursor = HEADER_LEN + (m_index * (sizeof(uint64_t) + sizeof(uint32_t)));
...
HintfileReader::HintfileReader(HintfileReader&& o) noexcept : m_file{o.m_file.release()}, m_stop_height{o.m_stop_height}, m_height_to_file_pos{std::move(o.m_height_to_file_pos)} {};
...
void Context::ApplyHints(HintfileReader reader)
{
