...
class TxOrphanageImpl final : public TxOrphanage {
    ...
    struct Announcement
    {
         ...
         * announcements with m_reconsider=true. mutable because it does not affect the ByWtxid ordering. */
        mutable bool m_reconsider{false};
         ...
         * transaction itself, each entry in m_by_wtxid, and each entry in m_outpoint_to_orphan_wtxids. We use weight because
         ...
         * The computation time is a function of the number of entries in m_by_wtxid (thus 1 per announcement) and the
    ...
    // Primary index: sorted by (wtxid, announcer). Owns the Announcement objects.
    // is_transparent enables lower_bound/upper_bound/find with ByWtxidView keys directly.
    using ByWtxidView = std::pair<Wtxid, NodeId>;
    struct WtxidPeerCmp {
        using is_transparent = void;
        static ByWtxidView KeyOf(const Announcement& a) { return {a.m_tx->GetWitnessHash(), a.m_announcer}; }
        bool operator()(const Announcement& a, const Announcement& b) const { return KeyOf(a) < KeyOf(b); }
        bool operator()(const ByWtxidView& k, const Announcement& b) const { return k < KeyOf(b); }
        bool operator()(const Announcement& a, const ByWtxidView& k) const { return KeyOf(a) < k; }
    };

    // Secondary index key: (peer, reconsider, sequence). Sorts non-reconsiderable oldest-first per peer.
    using ByPeerView = std::tuple<NodeId, bool, SequenceNumber>;

    using WtxidSet = std::set<Announcement, WtxidPeerCmp>;
    using WtxidIter = WtxidSet::iterator;
    // PeerMap value: iterator into m_by_wtxid plus stats cached to avoid dereferencing
    // the WtxidSet node in the hot Erase path. These values are const for a given Announcement.
    struct PeerEntry {
        WtxidIter wtxid_it;
        TxOrphanage::Usage mem_usage;
        TxOrphanage::Count latency_score;
    };
    using PeerMap = std::map<ByPeerView, PeerEntry>;
    using PeerIter = PeerMap::iterator;

    /** Primary container: one entry per announcement, sorted by (wtxid, announcer). */
    WtxidSet m_by_wtxid;
    /** Secondary index: maps (peer, reconsider, seq) -> iterator into m_by_wtxid. */
    PeerMap m_by_peer;
    ...
    /** Number of unique orphans by wtxid. Less than or equal to the number of entries in m_by_wtxid. */
     ...
     * the number of entries in m_by_wtxid. */
    TxOrphanage::Count m_unique_rounded_input_scores{0};

    /** Index from the parents' outputs to wtxids that exist in m_by_wtxid. Used to find children of
    ...
    /** Erase the announcement pointed to by both iterators. O(1) amortized erase from both indexes. */
    void Erase(WtxidIter it, PeerIter peer_it);

    /** Erase by WtxidIter only; looks up the PeerIter via key search. Use when peer_it is unavailable. */
    void Erase(WtxidIter it);

    /** Erase by wtxid (all announcers). */
    bool EraseTxInternal(const Wtxid& wtxid);

    /** Return true if there is exactly one announcement with the same wtxid as *it. */
    bool IsUnique(WtxidIter it) const;
    ...
    /** Update m_reconsider on *it and keep m_by_peer consistent (erase old key, reinsert with new key). */
    void SetReconsider(WtxidIter it, bool value);
...
void TxOrphanageImpl::Erase(WtxidIter wtxid_it, PeerIter peer_it)
{
    // Read cached stats and key fields from peer_it to avoid dereferencing wtxid_it
    // until we actually need to access the WtxidSet node (IsUnique, outpoint cleanup).
    const NodeId announcer{std::get<0>(peer_it->first)};
    const bool reconsider{std::get<1>(peer_it->first)};
    const TxOrphanage::Usage mem_usage{peer_it->second.mem_usage};
    const TxOrphanage::Count latency_score{peer_it->second.latency_score};

    // Update m_peer_orphanage_info and clean up entries if they point to an empty struct.
    auto dos_it = m_peer_orphanage_info.find(announcer);
    Assume(dos_it != m_peer_orphanage_info.end());
    dos_it->second.m_total_usage -= mem_usage;
    dos_it->second.m_total_latency_score -= latency_score;
    dos_it->second.m_count_announcements -= 1;
    if (dos_it->second.m_count_announcements == 0) m_peer_orphanage_info.erase(dos_it);

    if (IsUnique(wtxid_it)) {
        m_unique_orphans -= 1;
        m_unique_rounded_input_scores -= latency_score - 1;
        m_unique_orphan_usage -= mem_usage;

        // Remove references in m_outpoint_to_orphan_wtxids
        const auto& wtxid{wtxid_it->m_tx->GetWitnessHash()};
        for (const auto& input : wtxid_it->m_tx->vin) {
    ...
    // If this was the reconsiderable announcement for its wtxid, clear it.
    if (reconsider) m_reconsiderable_wtxids.erase(wtxid_it->m_tx->GetWitnessHash());

    m_by_peer.erase(peer_it);   // O(1) amortized iterator erase
    m_by_wtxid.erase(wtxid_it);
}

void TxOrphanageImpl::Erase(WtxidIter it)
{
    // Look up the peer iterator by key when the caller only has the wtxid iterator.
    auto peer_it = m_by_peer.find(ByPeerView{it->m_announcer, it->m_reconsider, it->m_entry_sequence});
    Assume(peer_it != m_by_peer.end());
    Erase(it, peer_it);
}

void TxOrphanageImpl::SetReconsider(WtxidIter it, bool value)
{
    // The ByPeer key includes m_reconsider, so we must erase old entry and reinsert with new key.
    auto old_peer_it = m_by_peer.find(ByPeerView{it->m_announcer, it->m_reconsider, it->m_entry_sequence});
    Assume(old_peer_it != m_by_peer.end());
    PeerEntry entry = old_peer_it->second;
    m_by_peer.erase(old_peer_it);
    it->m_reconsider = value;
    m_by_peer.emplace(ByPeerView{it->m_announcer, it->m_reconsider, it->m_entry_sequence}, std::move(entry));
}

bool TxOrphanageImpl::IsUnique(WtxidIter it) const
{
    if (it == m_by_wtxid.end()) return false;
    // m_by_wtxid is sorted by (wtxid, peer), so same-wtxid entries are adjacent.
    if (std::next(it) != m_by_wtxid.end() && std::next(it)->m_tx->GetWitnessHash() == it->m_tx->GetWitnessHash()) return false;
    if (it != m_by_wtxid.begin() && std::prev(it)->m_tx->GetWitnessHash() == it->m_tx->GetWitnessHash()) return false;
    return true;
}
...
TxOrphanage::Count TxOrphanageImpl::CountAnnouncements() const { return m_by_wtxid.size(); }
...
bool TxOrphanageImpl::AddTx(const CTransactionRef& tx, NodeId peer)
{
    ...
    auto [it, inserted] = m_by_wtxid.emplace(tx, peer, m_current_sequence);
    // If the announcement (same wtxid, same peer) already exists, emplacement fails. Return false.
    if (!inserted) return false;

    // Insert into secondary index. m_reconsider starts false.
    m_by_peer.emplace(ByPeerView{it->m_announcer, it->m_reconsider, it->m_entry_sequence},
                      PeerEntry{it, it->GetMemUsage(), it->GetLatencyScore()});
    ++m_current_sequence;
    ...
    if (brand_new) {
        ...
        LogDebug(BCLog::TXPACKAGES, "stored orphan tx %s (wtxid=%s), weight: %u (mapsz %u outsz %u)\n",
                    txid.ToString(), wtxid.ToString(), sz, m_by_wtxid.size(), m_outpoint_to_orphan_wtxids.size());
        Assume(IsUnique(it));
    } else {
    ...
    // DoS prevention: do not allow m_by_wtxid to grow unbounded (see CVE-2012-3789)
    LimitOrphans();
    return brand_new;
}
...
{
    auto it = m_by_wtxid.lower_bound(ByWtxidView{wtxid, MIN_PEER});

    // Do nothing if this transaction isn't already present.
    if (it == m_by_wtxid.end()) return false;
    ...
    auto [new_it, inserted] = m_by_wtxid.emplace(ptx, peer, m_current_sequence);
    // If the announcement (same wtxid, same peer) already exists, emplacement fails. Return false.
    if (!inserted) return false;

    m_by_peer.emplace(ByPeerView{new_it->m_announcer, new_it->m_reconsider, new_it->m_entry_sequence},
                      PeerEntry{new_it, new_it->GetMemUsage(), new_it->GetLatencyScore()});
    ++m_current_sequence;

    auto& peer_info = m_peer_orphanage_info.try_emplace(peer).first->second;
    peer_info.Add(*new_it);

    const auto& txid = ptx->GetHash();
    LogDebug(BCLog::TXPACKAGES, "added peer=%d as announcer of orphan tx %s (wtxid=%s)\n",
                peer, txid.ToString(), wtxid.ToString());

    Assume(!IsUnique(new_it));
...
{
    auto it = m_by_wtxid.lower_bound(ByWtxidView{wtxid, MIN_PEER});
    if (it == m_by_wtxid.end() || it->m_tx->GetWitnessHash() != wtxid) return false;

    auto it_end = m_by_wtxid.upper_bound(ByWtxidView{wtxid, MAX_PEER});
    unsigned int num_ann{0};
    const auto txid = it->m_tx->GetHash();
    while (it != it_end) {
        WtxidIter next = std::next(it);
        Erase(it);
        it = next;
        num_ann += 1;
    }
...
{
    auto it = m_by_peer.lower_bound(ByPeerView{peer, false, 0});
    if (it == m_by_peer.end() || it->second.wtxid_it->m_announcer != peer) return;

    unsigned int num_ann{0};
    while (it != m_by_peer.end() && it->second.wtxid_it->m_announcer == peer) {
        WtxidIter wtxid_it = it->second.wtxid_it;
        PeerIter erase_it = it++;
        Erase(wtxid_it, erase_it);
        num_ann += 1;
    }
...
void TxOrphanageImpl::LimitOrphans()
{
    ...
    do {
        ...
        auto it_ann = m_by_peer.lower_bound(ByPeerView{worst_peer, false, 0});
        unsigned int num_erased_this_round{0};
        unsigned int starting_num_ann{it_worst_peer->second.m_count_announcements};
        while (NeedsTrim()) {
            if (!Assume(it_ann != m_by_peer.end())) break;
            if (!Assume(it_ann->second.wtxid_it->m_announcer == worst_peer)) break;

            WtxidIter wtxid_it = it_ann->second.wtxid_it;
            PeerIter erase_it = it_ann++;
            Erase(wtxid_it, erase_it);
            num_erased += 1;
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++) {
        ...
        if (it_by_prev != m_outpoint_to_orphan_wtxids.end()) {
            for (const auto& wtxid : it_by_prev->second) {
                ...
                auto it = m_by_wtxid.lower_bound(ByWtxidView{wtxid, MIN_PEER});
                if (!Assume(it != m_by_wtxid.end() && it->m_tx->GetWitnessHash() == wtxid)) continue;
                ...
                // Mark this orphan as ready to be reconsidered. Since we checked m_reconsiderable_wtxids above,
                // the selected announcement must currently have m_reconsider=false.
                Assume(!it->m_reconsider);
                SetReconsider(it, true);
...
{
    auto it = m_by_wtxid.lower_bound(ByWtxidView{wtxid, MIN_PEER});
    return it != m_by_wtxid.end() && it->m_tx->GetWitnessHash() == wtxid;
}

CTransactionRef TxOrphanageImpl::GetTx(const Wtxid& wtxid) const
{
    auto it = m_by_wtxid.lower_bound(ByWtxidView{wtxid, MIN_PEER});
    if (it != m_by_wtxid.end() && it->m_tx->GetWitnessHash() == wtxid) return it->m_tx;
    return nullptr;
}

bool TxOrphanageImpl::HaveTxFromPeer(const Wtxid& wtxid, NodeId peer) const
{
    return m_by_wtxid.find(ByWtxidView{wtxid, peer}) != m_by_wtxid.end();
}
...
{
    auto it = m_by_peer.lower_bound(ByPeerView{peer, true, 0});
    if (it != m_by_peer.end() && std::get<0>(it->first) == peer && std::get<1>(it->first)) {
        WtxidIter wtxid_it = it->second.wtxid_it;
        ...
        m_reconsiderable_wtxids.erase(wtxid_it->m_tx->GetWitnessHash());
        return wtxid_it->m_tx;
    }
    return nullptr;
}
...
{
    auto it = m_by_peer.lower_bound(ByPeerView{peer, true, 0});
    return it != m_by_peer.end() && std::get<0>(it->first) == peer && std::get<1>(it->first);
}

void TxOrphanageImpl::EraseForBlock(const CBlock& block)
{
    if (m_by_wtxid.empty()) return;
...
std::vector<CTransactionRef> TxOrphanageImpl::GetChildrenFromSamePeer(const CTransactionRef& parent, NodeId peer) const
{
    ...
    auto it_upper = m_by_peer.upper_bound(ByPeerView{peer, true, std::numeric_limits<uint64_t>::max()});
    auto it_lower = m_by_peer.lower_bound(ByPeerView{peer, false, 0});

    while (it_upper != it_lower) {
        --it_upper;
        if (!Assume(std::get<0>(it_upper->first) == peer)) break;
        // Check if this tx spends from parent.
        for (const auto& input : it_upper->second.wtxid_it->m_tx->vin) {
            if (input.prevout.hash == parent_txid) {
                children_found.emplace_back(it_upper->second.wtxid_it->m_tx);
                break;
            }
        }
    }
...
std::vector<TxOrphanage::OrphanInfo> TxOrphanageImpl::GetOrphanTransactions() const
{
    ...
    // m_by_wtxid is sorted by (wtxid, peer), so same-wtxid entries are adjacent.
    auto it = m_by_wtxid.begin();
    std::set<NodeId> this_orphan_announcers;
    while (it != m_by_wtxid.end()) {
        ...
        if (std::next(it) == m_by_wtxid.end() || std::next(it)->m_tx->GetWitnessHash() != it->m_tx->GetWitnessHash()) {
...
void TxOrphanageImpl::SanityCheck() const
{
    ...
    for (auto it = m_by_wtxid.begin(); it != m_by_wtxid.end(); ++it) {
    ...
    // orphan, and all wtxids referenced in m_outpoint_to_orphan_wtxids are also in m_by_wtxid.
    ...
    assert(m_by_wtxid.size() >= m_unique_orphans);
    assert(m_by_wtxid.size() <= m_peer_orphanage_info.size() * m_unique_orphans);
    ...
    assert(summed_peer_latency_score >= m_unique_rounded_input_scores + m_by_wtxid.size());

    // Secondary index must be consistent with primary.
    assert(m_by_peer.size() == m_by_wtxid.size());
    for (const auto& [peer_key, entry] : m_by_peer) {
        assert(std::get<0>(peer_key) == entry.wtxid_it->m_announcer);
        assert(std::get<1>(peer_key) == entry.wtxid_it->m_reconsider);
        assert(std::get<2>(peer_key) == entry.wtxid_it->m_entry_sequence);
        assert(entry.mem_usage == entry.wtxid_it->GetMemUsage());
        assert(entry.latency_score == entry.wtxid_it->GetLatencyScore());
    }
...
TxOrphanage::Count TxOrphanageImpl::TotalLatencyScore() const { return m_unique_rounded_input_scores + m_by_wtxid.size(); }
