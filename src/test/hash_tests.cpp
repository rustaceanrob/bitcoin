...
TEST_SUITE_BEGIN(hash_tests)

FIXTURE_TEST_CASE(murmurhash3, BasicTestingSetup)
{

#define T(expected, seed, data) CHECK(MurmurHash3(seed, ParseHex(data)) == expected)
...
FIXTURE_TEST_CASE(siphash, BasicTestingSetup)
{
    CSipHasher hasher(0x0706050403020100ULL, 0x0F0E0D0C0B0A0908ULL);
    CHECK(hasher.Finalize() == 0x726fdb47dd0e0e31ull);
    static const unsigned char t0[1] = {0};
    hasher.Write(t0);
    CHECK(hasher.Finalize() == 0x74f839c593dc67fdull);
    static const unsigned char t1[7] = {1,2,3,4,5,6,7};
    hasher.Write(t1);
    CHECK(hasher.Finalize() == 0x93f5f5799a932462ull);
    hasher.Write(0x0F0E0D0C0B0A0908ULL);
    CHECK(hasher.Finalize() == 0x3f2acc7f57c29bdbull);
    static const unsigned char t2[2] = {16,17};
    hasher.Write(t2);
    CHECK(hasher.Finalize() == 0x4bc1b3f0968dd39cull);
    static const unsigned char t3[9] = {18,19,20,21,22,23,24,25,26};
    hasher.Write(t3);
    CHECK(hasher.Finalize() == 0x2f2e6163076bcfadull);
    static const unsigned char t4[5] = {27,28,29,30,31};
    hasher.Write(t4);
    CHECK(hasher.Finalize() == 0x7127512f72f27cceull);
    hasher.Write(0x2726252423222120ULL);
    CHECK(hasher.Finalize() == 0x0e3ea96b5304a7d0ull);
    hasher.Write(0x2F2E2D2C2B2A2928ULL);
    CHECK(hasher.Finalize() == 0xe612a3cb9ecba951ull);

    CHECK(PresaltedSipHasher(0x0706050403020100ULL, 0x0F0E0D0C0B0A0908ULL)(uint256{"1f1e1d1c1b1a191817161514131211100f0e0d0c0b0a09080706050403020100"}) == 0x7127512f72f27cceull);

    // Check test vectors from spec, one byte at a time
    CSipHasher hasher2(0x0706050403020100ULL, 0x0F0E0D0C0B0A0908ULL);
    for (uint8_t x=0; x<std::size(siphash_4_2_testvec); ++x)
    {
        CHECK(hasher2.Finalize() == siphash_4_2_testvec[x]);
    ...
    {
        CHECK(hasher3.Finalize() == siphash_4_2_testvec[x]);
    ...
    CHECK(PresaltedSipHasher(1, 2)(ss.GetHash()) == 0x79751e980c2a0a35ULL);
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        sip256.Write(x);
        CHECK(PresaltedSipHasher(k0, k1)(x) == sip256.Finalize());
        ...
        WriteLE32(nb, n);
        sip288.Write(nb);
        CHECK(PresaltedSipHasher(k0, k1)(x, n) == sip288.Finalize());
    }
}
