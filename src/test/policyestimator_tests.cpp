...
TEST_SUITE_BEGIN(policyestimator_tests)

FIXTURE_TEST_CASE(BlockPolicyEstimates, ChainTestingSetup)
{
    ...
    while (blocknum < 200) {
        ...
        if (blocknum == 3) {
            ...
            CHECK(feeEst.estimateFee(1) == CFeeRate(0));
            CHECK(feeEst.estimateFee(2).GetFeePerK() < 9*baseRate.GetFeePerK() + deltaFee);
            CHECK(feeEst.estimateFee(2).GetFeePerK() > 9*baseRate.GetFeePerK() - deltaFee);
        }
    }
    ...
    for (int i = 1; i < 10;i++) {
        ...
        if (i > 2) { // Fee estimates should be monotonically decreasing
            CHECK(origFeeEst[i-1] <= origFeeEst[i-2]);
        }
        int mult = 11-i;
        if (i % 2 == 0) { //At scale 2, test logic is only correct for even targets
            CHECK(origFeeEst[i-1] < mult*baseRate.GetFeePerK() + deltaFee);
            CHECK(origFeeEst[i-1] > mult*baseRate.GetFeePerK() - deltaFee);
        }
    }
    ...
    CHECK(feeEst.estimateFee(1) == CFeeRate(0));
    for (int i = 2; i < 10;i++) {
        CHECK(feeEst.estimateFee(i).GetFeePerK() < origFeeEst[i-1] + deltaFee);
        CHECK(feeEst.estimateFee(i).GetFeePerK() > origFeeEst[i-1] - deltaFee);
    }
    ...
    for (int i = 1; i < 10;i++) {
        CHECK((feeEst.estimateFee(i) == CFeeRate(0) || feeEst.estimateFee(i).GetFeePerK() > origFeeEst[i-1] - deltaFee));
    }
