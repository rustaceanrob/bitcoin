...
TEST_SUITE_BEGIN(blockmanager_tests)

FIXTURE_TEST_CASE(blockmanager_find_block_pos, BasicTestingSetup)
{
    ...
    LOCK(::cs_main);
    CHECK(blockman.WriteBlock(params->GenesisBlock(), 0).nPos == STORAGE_HEADER_BYTES);
    ...
    CHECK(actual.nPos == STORAGE_HEADER_BYTES + ::GetSerializeSize(TX_WITH_WITNESS(params->GenesisBlock())) + STORAGE_HEADER_BYTES);
}

FIXTURE_TEST_CASE(blockmanager_scan_unlink_already_pruned_files, TestChain100Setup)
{
...
FIXTURE_TEST_CASE(blockmanager_block_data_availability, TestChain100Setup)
{
    ...
    CHECK(&blockman.GetFirstBlock(tip, BLOCK_HAVE_DATA) == chainman->ActiveChain()[0]);
    ...
    CHECK(&blockman.GetFirstBlock(tip, BLOCK_HAVE_DATA) == first_available_block);
...
FIXTURE_TEST_CASE(blockmanager_block_data_part, TestChain100Setup)
{
    LOCK(::cs_main);
    auto& chainman{m_node.chainman};
    auto& blockman{chainman->m_blockman};
    ...
    const auto expect_part{[&](size_t offset, size_t size) {
        ...
        CHECK_EQUAL_RANGES(part, std::ranges::subrange(block->begin() + offset, block->begin() + offset + size));
    }};
...
FIXTURE_TEST_CASE(blockmanager_block_data_part_error, TestChain100Setup)
{
    LOCK(::cs_main);
    auto& chainman{m_node.chainman};
    auto& blockman{chainman->m_blockman};
    ...
    const auto expect_part_error{[&](size_t offset, size_t size) {
        auto res{blockman.ReadRawBlock(tip_block_pos, std::pair{offset, size})};
        CHECK(!res);
        CHECK(res.error() == node::ReadRawError::BadPartRange);
    }};
...
FIXTURE_TEST_CASE(blockmanager_readblock_hash_mismatch, TestingSetup)
{
    CBlockIndex index;
    {
        LOCK(cs_main);
...
FIXTURE_TEST_CASE(blockmanager_flush_block_file, BasicTestingSetup)
{
    KernelNotifications notifications{Assert(m_node.shutdown_request), m_node.exit_status, *Assert(m_node.warnings)};
...
FIXTURE_TEST_CASE(prune_lock_update_and_delete, TestingSetup)
{
    LOCK(::cs_main);
    auto& chainman{*Assert(m_node.chainman)};
    auto& blockman{chainman.m_blockman};
