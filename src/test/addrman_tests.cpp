...
static CNetAddr ResolveIP(const std::string& ip)
{
    ...
    CHECK(addr.has_value(), strprintf("failed to resolve: %s", ip));
...
static CService ResolveService(const std::string& ip, uint16_t port = 0)
{
    ...
    CHECK(serv.has_value(), strprintf("failed to resolve: %s:%i", ip, port));
    return serv.value_or(CService{});
}

TEST_SUITE_BEGIN(addrman_tests)

FIXTURE_TEST_CASE(addrman_simple, BasicTestingSetup)
{
    ...
    CHECK(addrman->Size() == 0U);
    auto addr_null = addrman->Select().first;
    CHECK(addr_null.ToStringAddrPort() == "[::]:0");

    // Test: Does Addrman::Add work as expected.
    CService addr1 = ResolveService("250.1.1.1", 8333);
    CHECK(addrman->Add({CAddress(addr1, NODE_NONE)}, source));
    CHECK(addrman->Size() == 1U);
    auto addr_ret1 = addrman->Select().first;
    CHECK(addr_ret1.ToStringAddrPort() == "250.1.1.1:8333");

    // Test: Does IP address deduplication work correctly.
    //  Expected dup IP should not be added.
    CService addr1_dup = ResolveService("250.1.1.1", 8333);
    CHECK(!addrman->Add({CAddress(addr1_dup, NODE_NONE)}, source));
    CHECK(addrman->Size() == 1U);
    ...
    CHECK(addrman->Add({CAddress(addr2, NODE_NONE)}, source));
    CHECK((addrman->Size() >= 1));
    ...
    CHECK(addrman->Add(vAddr, source));
    CHECK((addrman->Size() >= 1));
}
...
FIXTURE_TEST_CASE(addrman_terrible_many_failures, BasicTestingSetup)
{
    ...
    CHECK(addrman->Add({addr}, source));
    CHECK(addrman->Good(addr));
    ...
    CHECK(addrman->Add({addr_helper}, source));
    CHECK(addrman->Good(addr_helper));
    ...
    CHECK(filtered.size() == 1U);
    CHECK(filtered[0].ToStringAddrPort() == "251.252.2.3:8333");

    std::vector<CAddress> unfiltered{addrman->GetAddr(/*max_addresses=*/0, /*max_pct=*/0, /*network=*/std::nullopt, /*filtered=*/false)};
    CHECK(unfiltered.size() == 2U);
}


FIXTURE_TEST_CASE(addrman_penalty_self_announcement, BasicTestingSetup)
{
    FakeNodeClock clock{};
    auto addrman = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, GetCheckRatio(m_node));
    ...
    CHECK(addrman->Add({caddr2}, source2, time_penalty));
    ...
    CHECK((retrieved_addr2.nTime == base_time - time_penalty));
}

FIXTURE_TEST_CASE(addrman_ports, BasicTestingSetup)
{
    auto addrman = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, GetCheckRatio(m_node));
    ...
    CHECK(addrman->Add({CAddress(addr1_port, NODE_NONE)}, source));
    CHECK(addrman->Size() == 2U);
    auto addr_ret2 = addrman->Select().first;
    CHECK((addr_ret2.ToStringAddrPort() == "250.1.1.1:8333" || addr_ret2.ToStringAddrPort() == "250.1.1.1:8334"));
    ...
    CHECK(addr_ret3.ToStringAddrPort() == "250.1.1.1:8333");
}

FIXTURE_TEST_CASE(addrman_select, BasicTestingSetup)
{
    auto addrman = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, GetCheckRatio(m_node));
    CHECK(!addrman->Select(false).first.IsValid());
    CHECK(!addrman->Select(true).first.IsValid());
    ...
    CHECK((addrman->Select(/*new_only=*/true).first == addr1));
    CHECK((addrman->Select(/*new_only=*/false).first == addr1));

    // Move address to the tried table
    CHECK(addrman->Good(CAddress(addr1, NODE_NONE)));

    CHECK(addrman->Size() == 1U);
    CHECK(!addrman->Select(/*new_only=*/true).first.IsValid());
    CHECK((addrman->Select().first == addr1));
    CHECK(addrman->Size() == 1U);

    // Add one address to the new table
    CService addr2 = ResolveService("250.3.1.1", 8333);
    CHECK(addrman->Add({CAddress(addr2, NODE_NONE)}, addr2));
    CHECK((addrman->Select(/*new_only=*/true).first == addr2));

    // Add two more addresses to the new table
    CService addr3 = ResolveService("250.3.2.2", 9999);
    CService addr4 = ResolveService("250.3.3.3", 9999);

    CHECK(addrman->Add({CAddress(addr3, NODE_NONE)}, addr2));
    CHECK(addrman->Add({CAddress(addr4, NODE_NONE)}, ResolveService("250.4.1.1", 8333)));
    ...
    CHECK(addrman->Add({CAddress(addr5, NODE_NONE)}, addr3));
    CHECK(addrman->Good(CAddress(addr5, NODE_NONE)));
    CHECK(addrman->Add({CAddress(addr6, NODE_NONE)}, addr3));
    CHECK(addrman->Good(CAddress(addr6, NODE_NONE)));
    CHECK(addrman->Add({CAddress(addr7, NODE_NONE)}, ResolveService("250.1.1.3", 8333)));
    CHECK(addrman->Good(CAddress(addr7, NODE_NONE)));

    // 6 addrs + 1 addr from last test = 7.
    CHECK(addrman->Size() == 7U);
...
FIXTURE_TEST_CASE(addrman_select_by_network, BasicTestingSetup)
{
    auto addrman = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, GetCheckRatio(m_node));
    CHECK(!addrman->Select(/*new_only=*/true, {NET_IPV4}).first.IsValid());
    CHECK(!addrman->Select(/*new_only=*/false, {NET_IPV4}).first.IsValid());
    ...
    CHECK((addrman->Select(/*new_only=*/true, {NET_IPV4}).first == addr1));
    CHECK((addrman->Select(/*new_only=*/false, {NET_IPV4}).first == addr1));
    CHECK(!addrman->Select(/*new_only=*/false, {NET_IPV6}).first.IsValid());
    CHECK(!addrman->Select(/*new_only=*/false, {NET_ONION}).first.IsValid());
    CHECK(!addrman->Select(/*new_only=*/false, {NET_I2P}).first.IsValid());
    CHECK(!addrman->Select(/*new_only=*/false, {NET_CJDNS}).first.IsValid());
    CHECK(!addrman->Select(/*new_only=*/true, {NET_CJDNS}).first.IsValid());
    CHECK((addrman->Select(/*new_only=*/false).first == addr1));

    // add I2P address to the new table
    CAddress i2p_addr;
    i2p_addr.SetSpecial("udhdrtrcetjm5sxzskjyr5ztpeszydbh4dpl3pl4utgqqw2v4jna.b32.i2p");
    CHECK(addrman->Add({i2p_addr}, source));

    CHECK((addrman->Select(/*new_only=*/true, {NET_I2P}).first == i2p_addr));
    CHECK((addrman->Select(/*new_only=*/false, {NET_I2P}).first == i2p_addr));
    CHECK((addrman->Select(/*new_only=*/false, {NET_IPV4}).first == addr1));
    std::unordered_set<Network> nets_with_entries = {NET_IPV4, NET_I2P};
    CHECK(addrman->Select(/*new_only=*/false, nets_with_entries).first.IsValid());
    CHECK(!addrman->Select(/*new_only=*/false, {NET_IPV6}).first.IsValid());
    CHECK(!addrman->Select(/*new_only=*/false, {NET_ONION}).first.IsValid());
    CHECK(!addrman->Select(/*new_only=*/false, {NET_CJDNS}).first.IsValid());
    std::unordered_set<Network> nets_without_entries = {NET_IPV6, NET_ONION, NET_CJDNS};
    CHECK(!addrman->Select(/*new_only=*/false, nets_without_entries).first.IsValid());

    // bump I2P address to tried table
    CHECK(addrman->Good(i2p_addr));

    CHECK(!addrman->Select(/*new_only=*/true, {NET_I2P}).first.IsValid());
    CHECK((addrman->Select(/*new_only=*/false, {NET_I2P}).first == i2p_addr));
    ...
    CHECK(addrman->Add({i2p_addr2}, source));

    CHECK((addrman->Select(/*new_only=*/true, {NET_I2P}).first == i2p_addr2));
    ...
    while (--counter > 0 && (!new_selected || !tried_selected)) {
        ...
        REQUIRE((selected == i2p_addr || selected == i2p_addr2));
    ...
    CHECK(new_selected);
    CHECK(tried_selected);
}

FIXTURE_TEST_CASE(addrman_select_special, BasicTestingSetup)
{
...
FIXTURE_TEST_CASE(addrman_new_collisions, BasicTestingSetup)
{
    auto addrman = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, GetCheckRatio(m_node));
    ...
    CHECK(addrman->Add({CAddress(addr1, NODE_NONE)}, source));
    CHECK(addrman->Size() == num_addrs - collisions);
...
FIXTURE_TEST_CASE(addrman_new_multiplicity, BasicTestingSetup)
{
    auto addrman = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, GetCheckRatio(m_node));
...
FIXTURE_TEST_CASE(addrman_tried_collisions, BasicTestingSetup)
{
    auto addrman = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, GetCheckRatio(m_node));
    ...
    while (num_addrs < 35) { // Magic number! 250.1.1.1 - 250.1.1.35 do not collide in tried with deterministic key = 1
        ...
        CHECK(addrman->Good(CAddress(addr, NODE_NONE)));
    ...
    CService addr2 = ResolveService("250.1.1." + ToString(++num_addrs));
    CHECK(addrman->Add({CAddress(addr2, NODE_NONE)}, source));
    CHECK(addrman->Good(CAddress(addr2, NODE_NONE)));
}


FIXTURE_TEST_CASE(addrman_getaddr, BasicTestingSetup)
{
    auto addrman = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, GetCheckRatio(m_node));
    ...
    CHECK(addrman->Add({addr1, addr3, addr5}, source1));
    CHECK(addrman->Add({addr2, addr4}, source2));

    CHECK(addrman->GetAddr(/*max_addresses=*/0, /*max_pct=*/0, /*network=*/std::nullopt).size() == 5U);
    // Net processing asks for 23% of addresses. 23% of 5 is 1 rounded down.
    CHECK(addrman->GetAddr(/*max_addresses=*/2500, /*max_pct=*/23, /*network=*/std::nullopt).size() == 1U);
    ...
    CHECK(vAddr.size() == percent23);
    CHECK(vAddr.size() == 461U);
    // (addrman.Size() < number of addresses added) due to address collisions.
    CHECK(addrman->Size() == 2006U);
}

FIXTURE_TEST_CASE(getaddr_unfiltered, BasicTestingSetup)
{
    auto addrman = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, GetCheckRatio(m_node));
    ...
    CHECK(addrman->Add({addr4}, source));

    // GetAddr filtered by quality (i.e. not IsTerrible) should only return addr1
    CHECK(addrman->GetAddr(/*max_addresses=*/0, /*max_pct=*/0, /*network=*/std::nullopt).size() == 1U);
    // Unfiltered GetAddr should return all addrs
    CHECK(addrman->GetAddr(/*max_addresses=*/0, /*max_pct=*/0, /*network=*/std::nullopt, /*filtered=*/false).size() == 4U);
}

FIXTURE_TEST_CASE(caddrinfo_get_tried_bucket_legacy, BasicTestingSetup)
{
    ...
    CHECK(info1.GetTriedBucket(nKey1, EMPTY_NETGROUPMAN) == 40);

    // Test: Make sure key actually randomizes bucket placement. A fail on
    //  this test could be a security issue.
    CHECK((info1.GetTriedBucket(nKey1, EMPTY_NETGROUPMAN) != info1.GetTriedBucket(nKey2, EMPTY_NETGROUPMAN)));

    // Test: Two addresses with same IP but different ports can map to
    //  different buckets because they have different keys.
    AddrInfo info2 = AddrInfo(addr2, source1);

    CHECK((info1.GetKey() != info2.GetKey()));
    CHECK((info1.GetTriedBucket(nKey1, EMPTY_NETGROUPMAN) != info2.GetTriedBucket(nKey1, EMPTY_NETGROUPMAN)));
    ...
    CHECK(buckets.size() == 160U);
}

FIXTURE_TEST_CASE(caddrinfo_get_new_bucket_legacy, BasicTestingSetup)
{
    ...
    CHECK(info1.GetNewBucket(nKey1, EMPTY_NETGROUPMAN) == 786);
    CHECK(info1.GetNewBucket(nKey1, source1, EMPTY_NETGROUPMAN) == 786);

    // Test: Make sure key actually randomizes bucket placement. A fail on
    //  this test could be a security issue.
    CHECK((info1.GetNewBucket(nKey1, EMPTY_NETGROUPMAN) != info1.GetNewBucket(nKey2, EMPTY_NETGROUPMAN)));

    // Test: Ports should not affect bucket placement in the addr
    AddrInfo info2 = AddrInfo(addr2, source1);
    CHECK((info1.GetKey() != info2.GetKey()));
    CHECK(info1.GetNewBucket(nKey1, EMPTY_NETGROUPMAN) == info2.GetNewBucket(nKey1, EMPTY_NETGROUPMAN));
...
// 101.7.0.0/16 AS7
// 101.8.0.0/16 AS8
FIXTURE_TEST_CASE(caddrinfo_get_tried_bucket, BasicTestingSetup)
{
    ...
    CHECK(info1.GetTriedBucket(nKey1, ngm_asmap) == 236);

    // Test: Make sure key actually randomizes bucket placement. A fail on
    //  this test could be a security issue.
    CHECK((info1.GetTriedBucket(nKey1, ngm_asmap) != info1.GetTriedBucket(nKey2, ngm_asmap)));
    ...
    CHECK((info1.GetKey() != info2.GetKey()));
    CHECK((info1.GetTriedBucket(nKey1, ngm_asmap) != info2.GetTriedBucket(nKey1, ngm_asmap)));
...
FIXTURE_TEST_CASE(caddrinfo_get_new_bucket, BasicTestingSetup)
{
    auto ngm_asmap{NetGroupManager::WithEmbeddedAsmap(test::data::asmap)};
    ...
    // Test: Make sure the buckets are what we expect
    CHECK(info1.GetNewBucket(nKey1, ngm_asmap) == 795);
    CHECK(info1.GetNewBucket(nKey1, source1, ngm_asmap) == 795);

    // Test: Make sure key actually randomizes bucket placement. A fail on
    //  this test could be a security issue.
    CHECK((info1.GetNewBucket(nKey1, ngm_asmap) != info1.GetNewBucket(nKey2, ngm_asmap)));

    // Test: Ports should not affect bucket placement in the addr
    AddrInfo info2 = AddrInfo(addr2, source1);
    CHECK((info1.GetKey() != info2.GetKey()));
    CHECK(info1.GetNewBucket(nKey1, ngm_asmap) == info2.GetNewBucket(nKey1, ngm_asmap));
...
FIXTURE_TEST_CASE(addrman_serialization, BasicTestingSetup)
{
    ...
    CHECK((addr_pos1.multiplicity != 0));
    CHECK((addr_pos2.multiplicity != 0));

    CHECK((addr_pos1 == addr_pos2));

    // deserializing asmaped peers.dat to non-asmaped addrman
    stream << *addrman_asmap1;
    stream >> *addrman_noasmap;
    AddressPosition addr_pos3 = addrman_noasmap->FindAddressEntry(addr).value();
    CHECK((addr_pos3.multiplicity != 0));
    CHECK((addr_pos1.bucket != addr_pos3.bucket));
    CHECK((addr_pos1.position != addr_pos3.position));
    ...
    CHECK((addr_pos4.multiplicity != 0));
    CHECK((addr_pos4.bucket != addr_pos3.bucket));
    CHECK((addr_pos4 == addr_pos2));
    ...
    CHECK((addr_pos5.bucket != addr_pos6.bucket));
    stream << *addrman_noasmap;
    stream >> *addrman_asmap1;
    ...
    CHECK((addr_pos7.bucket == addr_pos8.bucket));
    CHECK((addr_pos7.position != addr_pos8.position));
}

FIXTURE_TEST_CASE(remove_invalid, BasicTestingSetup)
{
    ...
    REQUIRE((pos + sizeof(new2_raw_replacement) <= stream.size()));
    ...
    REQUIRE((pos != std::string::npos));
    REQUIRE((pos + sizeof(tried2_raw_replacement) <= stream.size()));
...
FIXTURE_TEST_CASE(addrman_selecttriedcollision, BasicTestingSetup)
{
    ...
    CHECK((addrman->SelectTriedCollision().first.ToStringAddrPort() == "[::]:0"));
...
FIXTURE_TEST_CASE(addrman_noevict, BasicTestingSetup)
{
    auto addrman = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, GetCheckRatio(m_node));
    ...
    CHECK(addrman->Add({CAddress(addr36, NODE_NONE)}, source));
    CHECK(!addrman->Good(addr36));
    CHECK(addrman->SelectTriedCollision().first.ToStringAddrPort() == "250.1.1.19:0");
    ...
    CHECK(addrman->Add({CAddress(addr59, NODE_NONE)}, source));
    CHECK(!addrman->Good(addr59));

    CHECK(addrman->SelectTriedCollision().first.ToStringAddrPort() == "250.1.1.10:0");
...
FIXTURE_TEST_CASE(addrman_evictionworks, BasicTestingSetup)
{
    auto addrman = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, GetCheckRatio(m_node));
    ...
    CHECK(!addrman->Good(addr19));
    CHECK(addrman->SelectTriedCollision().first.ToStringAddrPort() == "250.1.1.36:0");
...
FIXTURE_TEST_CASE(load_addrman, BasicTestingSetup)
{
    ...
    CHECK(addrman.Add(addresses, source.value()));
    CHECK((addrman.Size() == 3));
    ...
    CHECK((addrman2.Size() == 0));
    ReadFromStream(addrman2, ssPeers2);
    CHECK((addrman2.Size() == 3));
}
...
FIXTURE_TEST_CASE(load_addrman_corrupted, BasicTestingSetup)
{
    ...
    AddrMan addrman2{EMPTY_NETGROUPMAN, !DETERMINISTIC, GetCheckRatio(m_node)};
    CHECK((addrman2.Size() == 0));
    CHECK_THROWS_AS(ReadFromStream(addrman2, ssPeers2), std::ios_base::failure);
}

FIXTURE_TEST_CASE(addrman_update_address, BasicTestingSetup)
{
    ...
    std::vector<CAddress> vAddr1{addrman->GetAddr(/*max_addresses=*/0, /*max_pct=*/0, /*network=*/std::nullopt)};
    CHECK(vAddr1.size() == 1U);
    CHECK((vAddr1.at(0).nTime == start_time));
    CHECK(vAddr1.at(0).nServices == NODE_NONE);
    ...
    CHECK(vAddr2.size() == 1U);
    CHECK((vAddr2.at(0).nTime >= start_time + 10000s));
    CHECK(vAddr2.at(0).nServices == NODE_NETWORK_LIMITED);
    ...
    CHECK(!addrman->Add({addr_v2}, source));
    std::vector<CAddress> vAddr3{addrman->GetAddr(/*max_addresses=*/0, /*max_pct=*/0, /*network=*/std::nullopt)};
    CHECK(vAddr3.size() == 1U);
    CHECK(vAddr3.at(0).nServices == (NODE_P2P_V2 | NODE_NETWORK_LIMITED));
    ...
    CHECK(vAddr4.size() == 1U);
    CHECK(vAddr4.at(0).nServices == NODE_NETWORK);

    // Promoting to Tried does not affect the service flags
    CHECK(addrman->Good(addr)); // addr has NODE_NONE
    std::vector<CAddress> vAddr5{addrman->GetAddr(/*max_addresses=*/0, /*max_pct=*/0, /*network=*/std::nullopt)};
    CHECK(vAddr5.size() == 1U);
    CHECK(vAddr5.at(0).nServices == NODE_NETWORK);
    ...
    CHECK(vAddr6.size() == 1U);
    CHECK(vAddr6.at(0).nServices == (NODE_NETWORK | NODE_P2P_V2));
}

FIXTURE_TEST_CASE(addrman_size, BasicTestingSetup)
{
    auto addrman = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, GetCheckRatio(m_node));
    const CNetAddr source = ResolveIP("252.2.2.2");

    // empty addrman
    CHECK(addrman->Size(/*net=*/std::nullopt, /*in_new=*/std::nullopt) == 0U);
    CHECK(addrman->Size(/*net=*/NET_IPV4, /*in_new=*/std::nullopt) == 0U);
    CHECK(addrman->Size(/*net=*/std::nullopt, /*in_new=*/true) == 0U);
    CHECK(addrman->Size(/*net=*/NET_IPV4, /*in_new=*/false) == 0U);

    // add two ipv4 addresses, one to tried and new
    const CAddress addr1{ResolveService("250.1.1.1", 8333), NODE_NONE};
    CHECK(addrman->Add({addr1}, source));
    CHECK(addrman->Good(addr1));
    const CAddress addr2{ResolveService("250.1.1.2", 8333), NODE_NONE};
    CHECK(addrman->Add({addr2}, source));

    CHECK(addrman->Size(/*net=*/std::nullopt, /*in_new=*/std::nullopt) == 2U);
    CHECK(addrman->Size(/*net=*/NET_IPV4, /*in_new=*/std::nullopt) == 2U);
    CHECK(addrman->Size(/*net=*/std::nullopt, /*in_new=*/true) == 1U);
    CHECK(addrman->Size(/*net=*/std::nullopt, /*in_new=*/false) == 1U);
    CHECK(addrman->Size(/*net=*/NET_IPV4, /*in_new=*/true) == 1U);
    CHECK(addrman->Size(/*net=*/NET_IPV4, /*in_new=*/false) == 1U);
    ...
    CHECK(addrman->Add({addr3}, source));
    CHECK(addrman->Size(/*net=*/std::nullopt, /*in_new=*/std::nullopt) == 3U);
    CHECK(addrman->Size(/*net=*/NET_IPV4, /*in_new=*/std::nullopt) == 2U);
    CHECK(addrman->Size(/*net=*/NET_I2P, /*in_new=*/std::nullopt) == 1U);
    CHECK(addrman->Size(/*net=*/NET_I2P, /*in_new=*/true) == 1U);
    CHECK(addrman->Size(/*net=*/std::nullopt, /*in_new=*/true) == 2U);
    CHECK(addrman->Size(/*net=*/std::nullopt, /*in_new=*/false) == 1U);
}
