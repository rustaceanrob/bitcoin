...
void TestPotentialDeadLockDetected(MutexType& mutex1, MutexType& mutex2)
{
    ...
    try {
        ...
        CHECK(std::string{e.what()} == "potential deadlock detected: mutex1 -> mutex2 -> mutex1");
        error_thrown = true;
    }
    CHECK(LockStackEmpty());
...
void TestDoubleLock(bool should_throw)
{
    ...
    MutexType m;
    {
        ...
        if (should_throw) {
            ...
            CHECK_NOTHROW(TestDoubleLock2(m));
        }
    }
    CHECK(LockStackEmpty());
...
TEST_SUITE_BEGIN(sync_tests)

TEST_CASE(potential_deadlock_detected)
{
    #ifdef DEBUG_LOCKORDER
...
#ifdef DEBUG_LOCKORDER
TEST_CASE(double_lock_mutex)
{
    TestDoubleLock<Mutex>(/*should_throw=*/true);
}

TEST_CASE(double_lock_recursive_mutex)
{
    TestDoubleLock<RecursiveMutex>(/*should_throw=*/false);
}
#endif /* DEBUG_LOCKORDER */

TEST_CASE(inconsistent_lock_order_detected)
{
#ifdef DEBUG_LOCKORDER
    bool prev = g_debug_lockorder_abort;
    g_debug_lockorder_abort = false;
