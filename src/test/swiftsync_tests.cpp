...
BOOST_AUTO_TEST_CASE(swiftsync_hintfile_test)
{
    const fs::path temppath = fsbridge::AbsPathJoin(gArgs.GetDataDirNet(), fs::u8path("test.hints"));
    {
        ...
        AutoFile afile{file};
        swiftsync::HintfileWriter writer{afile, 4};
        BOOST_CHECK(writer.WriteNextUnspents(unspent_block_1));
        BOOST_CHECK(writer.WriteNextUnspents(unspent_block_2));
        BOOST_CHECK(writer.WriteNextUnspents(unspent_block_3));
        BOOST_CHECK(writer.WriteNextUnspents(unspent_block_4));
    }
    FILE* file{fsbridge::fopen(temppath, "rb")};
    AutoFile afile{file};
    swiftsync::HintfileReader reader{afile};
