...
void CheckUnparsable(const std::string& prv, const std::string& pub, const std::string& expected_error)
{
    ...
    CHECK(parse_priv.empty(), prv);
    CHECK(parse_pub.empty(), pub);
...
void CheckInferRaw(const CScript& script)
{
    ...
    CHECK(desc->ToString().rfind("raw(", 0) == 0U);
}
             ...
             std::optional<std::string> expected_prv = std::nullopt, std::optional<std::string> expected_pub = std::nullopt, int desc_index = 0)
{
    ...
    CHECK((is_input_size_info_set || is_nontop_or_nonsolvable), prv);
    ...
    CHECK((is_combo || parse_priv->ScriptSize() == scripts[0][0].size() / 2), "Invalid ScriptSize() for " + prv);
    ...
    if (expected_pub) {
        ...
        CHECK(EqualDescriptor(pub, pub1), "Private ser: " + pub1 + " Public desc: " + pub);
        CHECK(EqualDescriptor(pub, pub2), "Public ser: " + pub2 + " Public desc: " + pub);
    }

    // Check that the COMPAT identifier did not change
    if (op_desc_id) {
        CHECK(DescriptorID(*parse_priv) == *op_desc_id, "DescriptorID() " + DescriptorID(*parse_priv).ToString() + " does not match for priv " + prv);
    }
    ...
    if (!(flags & MISSING_PRIVKEYS)) {
        ...
        if (expected_prv) {
            CHECK(EqualDescriptor(*expected_prv, prv1), "Private ser: " + prv1 + "Private desc: " + *expected_prv);
        } else {
            CHECK(EqualDescriptor(prv, prv1), "Private ser: " + prv1 + " Private desc: " + prv);
        }
        ...
        CHECK(EqualSigningProviders(priv_prov, pub_prov), "Private desc: " + prv + " Pub desc: " + pub);
        ...
        CHECK(prv == prv_str.substr(0, prv_str.length() - checksum_len), prv);
    }

    // Check that private can produce the normalized descriptors
    std::string norm1;
    CHECK(parse_priv->ToNormalizedString(keys_priv, norm1));
    CHECK(EqualDescriptor(norm1, norm_pub), "priv->ToNormalizedString(): " + norm1 + " Norm. desc: " + norm_pub);
    CHECK(parse_pub->ToNormalizedString(keys_priv, norm1));
    CHECK(EqualDescriptor(norm1, norm_pub), "pub->ToNormalizedString(): " + norm1 + " Norm. desc: " + norm_pub);

    // Check whether IsRange on both returns the expected result
    CHECK(parse_pub->IsRange() == ((flags & RANGE) != 0));
    ...
    CHECK(key_exprs + 1 == parse_pub->GetKeyCount(), "Pub desc: " + pub);
    CHECK(key_exprs == parse_priv->GetMaxKeyExpr(), "Priv desc: " + prv);
        ...
        for (int t = 0; t < 2; ++t) {
            ...
            if ((flags & RANGE) && !(flags & (DERIVE_HARDENED))) {
                ...
                if (flags & MUSIG_DERIVATION) {
                    ...
                    if (flags & MIXED_PUBKEYS) {
                        CHECK(num_xpubs == count_pks);
                    } else {
                        CHECK(script_provider_cached.origins.size() == count_pks);
                    }
                }
            } else if (num_xpubs > 0) {
                // For ranged, hardened derivation, or not ranged, but has an xpub, all of the keys should appear in the cache
                CHECK(der_xpub_cache.size() + parent_xpub_cache.size() == num_xpubs);
                if (!(flags & MIXED_PUBKEYS)) {
                    if (flags & UNIQUE_XPUBS) {
                        CHECK(script_provider_cached.origins.size() == num_unique_xpubs);
                    } else {
                        CHECK(script_provider_cached.origins.size() == num_xpubs);
                    }
                }
                ...
                if (flags & MUSIG_DERIVATION && !(flags & MIXED_PUBKEYS)) {
                    ...
                    CHECK(2U * count_pks == pubkeys.size());
                    if (flags & UNIQUE_XPUBS) {
                        CHECK(2 * num_unique_xpubs == pubkeys.size());
                    } else {
                        CHECK(2 * num_xpubs == pubkeys.size());
                    }
                } else {
                    if (flags & MUSIG) count_pks++; // One extra key for the aggregate key that is not in the cache
                    if (flags & MIXED_PUBKEYS) {
            ...
            for (size_t n = 0; n < spks.size(); ++n) {
                ...
                if (flags & (SIGNABLE | SIGNABLE_FAILS)) {
                    ...
                    CHECK(prod_sig_res == !(flags & SIGNABLE_FAILS), prv);
                }
                ...
                CHECK(spks_inferred.size() == 1U);
                CHECK(spks_inferred[0] == spks[n]);
                CHECK(InferDescriptor(spks_inferred[0], provider_inferred)->IsSolvable() == !(flags & UNSOLVABLE));
            ...
            for (const auto& origin : script_provider.origins) {
                CHECK(paths.contains(origin.second.second.path), "Unexpected key path: " + prv);
    ...
    CHECK(left_paths.empty(), "Not all expected key paths found: " + prv);
}
...
void CheckInferDescriptor(const std::string& script_hex, const std::string& expected_desc, const std::vector<std::string>& hex_scripts, const std::vector<std::pair<std::string, std::string>>& origin_pubkeys)
{
    ...
    CHECK(desc->ToString() == expected_desc + "#" + checksum);
}

}

TEST_SUITE_BEGIN(descriptor_tests)

FIXTURE_TEST_CASE(descriptor_test, BasicTestingSetup)
{
...
FIXTURE_TEST_CASE(descriptor_literal_null_byte, BasicTestingSetup)
{
    ...
    REQUIRE(!descs.empty(), err);
}

FIXTURE_TEST_CASE(descriptor_older_warnings, BasicTestingSetup)
{
    ...
    // Height-based unsafe value (65536) should produce one warning.
    {
        ...
        REQUIRE(ws.size() == 1U);
        CHECK(ws[0] == strprintf("height-based relative locktime: older(%u) > 65535 blocks is unsafe", height_unsafe));
    }
    ...
    // Time-based unsafe value: add SEQUENCE_LOCKTIME_TYPE_FLAG (1<<22)
    {
        ...
        CHECK(warnings[0] == strprintf("time-based relative locktime: older(%u) > (65535 * 512) seconds is unsafe", time_unsafe));
    }
...
void CheckUnused(const std::string& prv, const std::string& pub)
{
    ...
    CHECK(parse_priv, error);
    CHECK(parse_pub, error);

    CHECK(parse_priv->GetOutputType() == std::nullopt);
    CHECK(parse_pub->GetOutputType() == std::nullopt);
