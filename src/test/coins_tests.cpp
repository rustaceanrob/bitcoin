...
public:
    ...
    void SelfTest(bool sanity_check = true) const
    {
        ...
        CHECK(GetCacheSize() == count);
        CHECK(DynamicMemoryUsage() == ret);
        if (sanity_check) {
            SanityCheck();
        }
    }
...
void SimulationTest(CCoinsView* base, bool fake_best_block)
{
    ...
    for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
        // Do a random modification.
        {
            ...
            if (test_havecoin_before) {
                CHECK(result_havecoin == !entry.IsSpent());
            }

            if (test_havecoin_after) {
                bool ret = stack.back()->HaveCoin(COutPoint(txid, 0));
                CHECK(ret == !entry.IsSpent());
            }
            ...
            if (m_rng.randrange(5) == 0 || coin.IsSpent()) {
                ...
                if (m_rng.randrange(16) == 0 && coin.IsSpent()) {
                    ...
                    CHECK(newcoin.out.scriptPubKey.IsUnspendable());
                ...
                coin.Clear();
                CHECK(stack.back()->SpendCoin(COutPoint(txid, 0)));
            }
        }
        ...
        if (m_rng.randrange(1000) == 1 || i == NUM_SIMULATION_ITERATIONS - 1) {
            for (const auto& entry : result) {
                ...
                CHECK(have == !coin.IsSpent());
                CHECK(coin == entry.second);
                if (coin.IsSpent()) {
                    missed_an_entry = true;
                } else {
                    CHECK(stack.back()->HaveCoinInCache(entry.first));
                    found_an_entry = true;
                }
            }
    ...
    // Verify coverage.
    CHECK(removed_all_caches);
    CHECK(reached_4_caches);
    CHECK(added_an_entry);
    CHECK(added_an_unspendable_entry);
    CHECK(removed_an_entry);
    CHECK(updated_an_entry);
    CHECK(found_an_entry);
    CHECK(missed_an_entry);
    CHECK(uncached_an_entry);
    CHECK(flushed_without_erase);
}
}; // struct CacheTest

TEST_SUITE_BEGIN(coins_tests_base)

// Run the above simulation for multiple base types.
FIXTURE_TEST_CASE(coins_cache_base_simulation_test, CacheTest)
{
...
TEST_SUITE_BEGIN(coins_tests_dbbase)

FIXTURE_TEST_CASE(coins_cache_dbbase_simulation_test, CacheTest)
{
...
FIXTURE_TEST_CASE(updatecoins_simulation_test, UpdateTest)
{
    ...
    for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
        ...
        if (randiter % 20 < 19) {
            ...
            // remove outputs
            CHECK(stack.back()->SpendCoin(utxod->first));
            // restore inputs
            if (!tx.IsCoinBase()) {
    ...
    // Verify coverage.
    CHECK(spent_a_duplicate_coinbase);
}

FIXTURE_TEST_CASE(ccoins_serialization, BasicTestingSetup)
{
    // Good example
    Coin cc1;
    SpanReader{"97f23c835800816115944e077fe7c803cfa57f29b36bf87c1d35"_hex} >> cc1;
    CHECK(cc1.IsCoinBase() == false);
    CHECK(cc1.nHeight == 203998U);
    CHECK(cc1.out.nValue == CAmount{60000000000});
    CHECK(HexStr(cc1.out.scriptPubKey) == HexStr(GetScriptForDestination(PKHash(uint160("816115944e077fe7c803cfa57f29b36bf87c1d35"_hex_u8)))));

    // Good example
    Coin cc2;
    SpanReader{"8ddf77bbd123008c988f1a4a4de2161e0f50aac7f17e7f9555caa4"_hex} >> cc2;
    CHECK(cc2.IsCoinBase() == true);
    CHECK(cc2.nHeight == 120891U);
    CHECK(cc2.out.nValue == 110397);
    CHECK(HexStr(cc2.out.scriptPubKey) == HexStr(GetScriptForDestination(PKHash(uint160("8c988f1a4a4de2161e0f50aac7f17e7f9555caa4"_hex_u8)))));

    // Smallest possible example
    Coin cc3;
    SpanReader{"000006"_hex} >> cc3;
    CHECK(cc3.IsCoinBase() == false);
    CHECK(cc3.nHeight == 0U);
    CHECK(cc3.out.nValue == 0);
    CHECK(cc3.out.scriptPubKey.size() == 0U);

    // scriptPubKey that ends beyond the end of the stream
    try {
        Coin cc4;
        SpanReader{"000007"_hex} >> cc4;
        CHECK(false, "We should have thrown");
...
static void CheckAccessCoin(const CAmount base_value, const MaybeCoin& cache_coin, const MaybeCoin& expected)
{
    ...
    CHECK(coin.IsSpent() == !test.cache.GetCoin(OUTPOINT));
    test.cache.SelfTest(/*sanity_check=*/false);
    CHECK(GetCoinsMapEntry(test.cache.map()) == expected);
}

FIXTURE_TEST_CASE(ccoins_access, BasicTestingSetup)
{
...
FIXTURE_TEST_CASE(ccoins_spend, BasicTestingSetup)
{
...
static void CheckAddCoin(const CAmount base_value, const MaybeCoin& cache_coin, const CAmount modify_value, const CoinOrError& expected, const bool coinbase)
{
    ...
    if (auto* expected_coin{std::get_if<MaybeCoin>(&expected)}) {
        add_coin();
        test.cache.SelfTest();
        CHECK(GetCoinsMapEntry(test.cache.map()) == *expected_coin);
    } else {
        CHECK_EXCEPTION(add_coin(), std::logic_error, HasReason(std::get<std::string>(expected)));
    }
}

FIXTURE_TEST_CASE(ccoins_add, BasicTestingSetup)
{
...
static void CheckWriteCoins(const MaybeCoin& parent, const MaybeCoin& child, const CoinOrError& expected)
{
    ...
    if (auto* expected_coin{std::get_if<MaybeCoin>(&expected)}) {
        ...
        CHECK_EXCEPTION(write_coins(), std::logic_error, HasReason(std::get<std::string>(expected)));
    }
}

FIXTURE_TEST_CASE(ccoins_write, BasicTestingSetup)
{
    ...
    bool do_erasing_flush)
{
    ...
    CHECK(!base.HaveCoin(outp));
    CHECK(!view->HaveCoin(outp));
    ...
    CHECK(GetCoinsMapEntry(view->map(), outp) == CoinEntry(coin.out.nValue, CoinEntry::State::DIRTY_FRESH));
    ...
    CHECK(cache_usage == view->DynamicMemoryUsage());
    CHECK(cache_size == view->map().size());

    // --- 3. Ensuring the entry still exists in the cache and has been written to parent
    //
    CHECK(GetCoinsMapEntry(view->map(), outp) == CoinEntry(coin.out.nValue, CoinEntry::State::CLEAN)); // State should have been wiped.
    ...
    if (do_erasing_flush) {
        ...
        CHECK(view->DynamicMemoryUsage() <= cache_usage);
        // Size of the cache must go down though
        CHECK(view->map().size() < cache_size);

        // --- 5. Ensuring the entry is no longer in the cache
        //
        CHECK(!GetCoinsMapEntry(view->map(), outp));
        view->AccessCoin(outp);
        CHECK(GetCoinsMapEntry(view->map(), outp) == CoinEntry(coin.out.nValue, CoinEntry::State::CLEAN));
    }
    ...
    // --- 6. Spend the coin.
    //
    CHECK(view->SpendCoin(outp));

    // The coin should be in the cache, but spent and marked dirty.
    CHECK(GetCoinsMapEntry(view->map(), outp) == SPENT_DIRTY);
    CHECK(!view->HaveCoin(outp)); // Coin should be considered spent in `view`.
    CHECK(base.HaveCoin(outp));  // But coin should still be unspent in `base`.
    ...
    coin = MakeCoin();
    CHECK(!base.HaveCoin(outp));
    CHECK(!all_caches[0]->HaveCoin(outp));
    CHECK(!all_caches[1]->HaveCoin(outp));

    all_caches[0]->AddCoin(outp, std::move(coin), false);
    all_caches[0]->Sync();
    CHECK(base.HaveCoin(outp));
    CHECK(all_caches[0]->HaveCoin(outp));
    CHECK(!all_caches[1]->HaveCoinInCache(outp));

    CHECK(all_caches[1]->SpendCoin(outp));
    flush_all(/*erase=*/ false);
    CHECK(!base.HaveCoin(outp));
    CHECK(!all_caches[0]->HaveCoin(outp));
    CHECK(!all_caches[1]->HaveCoin(outp));
    ...
    CHECK(GetCoinsMapEntry(all_caches[0]->map(), outp) == CoinEntry(coin_val, CoinEntry::State::DIRTY_FRESH));
    // Base shouldn't have seen coin.
    CHECK(!base.HaveCoin(outp));

    CHECK(all_caches[0]->SpendCoin(outp));
    all_caches[0]->Sync();

    // Ensure there is no sign of the coin after spend/flush.
    CHECK(!GetCoinsMapEntry(all_caches[0]->map(), outp));
    CHECK(!all_caches[0]->HaveCoinInCache(outp));
    CHECK(!base.HaveCoin(outp));
}
}; // struct FlushTest

FIXTURE_TEST_CASE(ccoins_flush_behavior, FlushTest)
{
...
FIXTURE_TEST_CASE(coins_db_leveldb_layout, FlushTest)
{
    ...
    CHECK(level2_files(base) == 0);
    WITH_LOCK(::cs_main, return base.CompactFull()).wait();
    CHECK(level2_files(base) == 1);

    CHECK(*Assert(base.GetCoin(outpoint)) == coin);
    CHECK(base.GetBestBlock() == block_hash);
}

FIXTURE_TEST_CASE(coins_resource_is_used, BasicTestingSetup)
{
    CCoinsMapMemoryResource resource;
    ...
    {
        CCoinsMap map{0, CCoinsMap::hasher{}, CCoinsMap::key_equal{}, &resource};
        CHECK(memusage::DynamicUsage(map) >= resource.ChunkSizeBytes());
...
FIXTURE_TEST_CASE(ccoins_addcoin_exception_keeps_usage_balanced, BasicTestingSetup)
{
    ...
    CHECK_THROWS_AS(cache.AddCoin(outpoint, Coin{coin2}, /*possible_overwrite=*/false), std::logic_error);
    cache.SelfTest();

    CHECK(cache.AccessCoin(outpoint) == coin1);
}

FIXTURE_TEST_CASE(ccoins_emplace_duplicate_keeps_usage_balanced, BasicTestingSetup)
{
    CCoinsViewCacheTest cache{&CoinsViewEmpty::Get()};
...
FIXTURE_TEST_CASE(ccoins_reset_guard, BasicTestingSetup)
{
...
FIXTURE_TEST_CASE(ccoins_peekcoin, BasicTestingSetup)
{
    CCoinsViewTest base{m_rng};
