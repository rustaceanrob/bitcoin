...
TEST_SUITE_BEGIN("private_broadcast_tests")
...
FIXTURE_TEST_CASE("basic", BasicTestingSetup)
{
    ...
    REQUIRE((tx1->GetWitnessHash() != tx2->GetWitnessHash()));
    ...
    CHECK(tx_for_recipient1 != tx_for_recipient2);
    ...
    CHECK(pb.GetTxForNode(recipient1).value() == tx_for_recipient1);
    CHECK(pb.GetTxForNode(recipient2).value() == tx_for_recipient2);
...
FIXTURE_TEST_CASE("stale_unpicked_tx", BasicTestingSetup)
{
    SetMockTime(Now<NodeSeconds>());
