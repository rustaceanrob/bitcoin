...
TEST_SUITE_BEGIN("peerman_tests")
...
FIXTURE_TEST_CASE("connections_desirable_service_flags", RegTestingSetup)
{
    ...
    CHECK((peerman->GetDesirableServiceFlags(peer_flags) == ServiceFlags(NODE_NETWORK | NODE_WITNESS)));
    ...
    CHECK((peerman->GetDesirableServiceFlags(peer_flags) == ServiceFlags(NODE_NETWORK_LIMITED | NODE_WITNESS)));
