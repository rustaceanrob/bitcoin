...
inline bool matches_filter(const TestCase& tc, std::string_view filter)
{
    ...
    return std::ranges::any_of(std::views::split(filter, ','), [&](auto piece) {
        const std::string_view f{piece.begin(), piece.end()};
        const auto pos = f.find('/');
        if (pos == std::string_view::npos) return (tc.suite && f == tc.suite) || f == tc.name;
        return tc.suite && f.substr(0, pos) == tc.suite && f.substr(pos + 1) == tc.name;
    });
}
...
{
    tfm::printf("Usage: %s [--run_test|-t <filter[,filter...]>] [--log_level|-l <none|error|info|all>] [--list_content] [-h|--help] [-- USER_ARGS...]\n"
...
inline ExitStatus parse_args(RuntimeOptions& opts, int argc, char** argv)
{
    ...
    for (int i{1}; i < argc; ++i) {
        ...
        if (arg.starts_with("-l")) {
            ...
            auto parsed = argv[i];
            auto log_level = to_log_level(parsed);
            if (log_level == LogLevel::Unknown) {
                opts.show_help = true;
                return ExitStatus::UnknownArgs;
            }
...
inline int run(int argc, char** argv)
{
    ...
    if (std::getenv("BITCOIN_TEST_RANDOM")) {
    ...
    for (const auto& test_case : registry()) {
        ...
        if (!ctx.did_fail()) {
            ...
            log(LogLevel::Error, "[FAIL] %s (%d/%d checks failed)\n",
                test_case.name, stats.failed, stats.checks);
        }
    }
    ...
    if (!opts.filter.empty() && summary.passed + summary.failed == 0) {
        tfm::printf("no test cases matching filter\n");
        return ExitStatus::Failure;
    }
 ...
 * Accepts stream-style expressions: WARN_MESSAGE("x=" << x); */
