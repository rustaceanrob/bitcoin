...
/** Default fixture for suites declared without one, and the fallback for
 * `TEST_CASE`s outside any suite. */
struct EmptyFixture {};

} // namespace framework

/** File-scope fallback. Each `TEST_SUITE_BEGIN(name, F)` shadows this with
 * a suite-local alias inside its own namespace, so multiple suites in one
 * TU don't collide. */
...
/** Opens `namespace suite { ... }` and registers the suite name via a
 * static-initializer side effect. An optional second argument declares a
 * suite-wide fixture alias picked up by `BOOST_AUTO_TEST_CASE`.
 *
 * Example: TEST_SUITE_BEGIN(my_tests, MyFixture)
 *          BOOST_AUTO_TEST_CASE(case_a) { ... }
 *          TEST_SUITE_END() */
...
/** Closes the namespace opened by TEST_SUITE_BEGIN and resets the suite name. */
...
/** Boost.Test compatibility shims so existing sources keep compiling.
 * Each macro is a plain 1:1 substitution. Structural transformations
 * (BOOST_FIXTURE_TEST_SUITE fixture promotion, namespace re-opening on
 * suite/namespace collisions) are handled by the migration script. */
...
#define BOOST_FIXTURE_TEST_CASE(name, F) FIXTURE_TEST_CASE(name, F)
