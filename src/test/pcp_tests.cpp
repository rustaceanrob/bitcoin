...
public:
          ...
          m_create_sock_orig{CreateSock}
    {
        ...
        REQUIRE(local_ipv4);
        REQUIRE(local_ipv6);
        REQUIRE(gateway_ipv4);
        REQUIRE(gateway_ipv6);
...
private:
    ...
    const TestOp &CurOp() const {
        REQUIRE(m_script_ptr < m_script.size());
...
TEST_SUITE_BEGIN(pcp_tests)

// NAT-PMP IPv4 good-weather scenario.
FIXTURE_TEST_CASE(natpmp_ipv4, PCPTestingSetup)
{
...
FIXTURE_TEST_CASE(pcp_ipv4, PCPTestingSetup)
{
    const std::vector<TestOp> script{
        {
            0ms, TestOp::SEND,
...
FIXTURE_TEST_CASE(pcp_ipv6, PCPTestingSetup)
{
    const std::vector<TestOp> script{
        {
            0ms, TestOp::SEND,
...
// PCP timeout.
FIXTURE_TEST_CASE(pcp_timeout, PCPTestingSetup)
{
    const std::vector<TestOp> script{};
    CreateSock = [this, &script](int domain, int type, int protocol) {
        if (domain == AF_INET && type == SOCK_DGRAM && protocol == IPPROTO_UDP) return std::make_unique<PCPTestSock>(default_local_ipv4, default_gateway_ipv4, script);
    ...
    REQUIRE(err);
    CHECK(*err == MappingError::NETWORK_ERROR);
}

// PCP failure receiving (router sends ICMP port closed).
FIXTURE_TEST_CASE(pcp_connrefused, PCPTestingSetup)
{
    const std::vector<TestOp> script{
        {
            0ms, TestOp::SEND,
...
FIXTURE_TEST_CASE(pcp_ipv6_timeout_success, PCPTestingSetup)
{
    const std::vector<TestOp> script{
        {
            0ms, TestOp::SEND,
...
FIXTURE_TEST_CASE(pcp_ipv4_fail_no_resources, PCPTestingSetup)
{
    const std::vector<TestOp> script{
        {
            0ms, TestOp::SEND,
    ...
    MappingError* err = std::get_if<MappingError>(&res);
    REQUIRE(err);
    CHECK(*err == MappingError::NO_RESOURCES);
}

// PCP IPv4 failure (test NATPMP downgrade scenario).
FIXTURE_TEST_CASE(pcp_ipv4_fail_unsupported_version, PCPTestingSetup)
{
    const std::vector<TestOp> script{
        {
            0ms, TestOp::SEND,
    ...
    MappingError* err = std::get_if<MappingError>(&res);
    REQUIRE(err);
    CHECK(*err == MappingError::UNSUPP_VERSION);
}

// NAT-PMP IPv4 protocol error scenarii.
FIXTURE_TEST_CASE(natpmp_protocol_error, PCPTestingSetup)
{
    ...
    MappingError* err = std::get_if<MappingError>(&res);
    REQUIRE(err);
    CHECK(*err == MappingError::PROTOCOL_ERROR);
...
FIXTURE_TEST_CASE(pcp_protocol_error, PCPTestingSetup)
{
    const std::vector<TestOp> script{
        {
            0ms, TestOp::SEND,
