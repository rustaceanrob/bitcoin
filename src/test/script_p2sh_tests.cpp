...
TEST_SUITE_BEGIN(script_p2sh_tests)

FIXTURE_TEST_CASE(sign, BasicTestingSetup)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        CHECK(keystore.AddCScript(standardScripts[i]));
    ...
    CHECK(IsStandardTx(CTransaction(txFrom), reason));
    ...
    {
        SignatureData empty;
        CHECK(SignSignature(keystore, CTransaction(txFrom), txTo[i], 0, SIGHASH_ALL, empty), strprintf("SignSignature %d", i));
    }
    ...
    for (int i = 0; i < 8; i++) {
        ...
        for (int j = 0; j < 8; j++)
        {
            ...
            if (i == j)
                CHECK(sigOK, strprintf("VerifySignature %d %d", i, j));
            else
                CHECK(!sigOK, strprintf("VerifySignature %d %d", i, j));
...
FIXTURE_TEST_CASE(norecurse, BasicTestingSetup)
{
    ScriptError err;
    ...
    CHECK(!Verify(scriptSig, p2sh, true, err));
    CHECK(err == SCRIPT_ERR_BAD_OPCODE, ScriptErrorString(err));
    ...
    CHECK(Verify(scriptSig2, p2sh2, true, err));
    CHECK(err == SCRIPT_ERR_OK, ScriptErrorString(err));
}

FIXTURE_TEST_CASE(set, BasicTestingSetup)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        CHECK(keystore.AddCScript(inner[i]));
    }
    ...
    for (int i = 0; i < 4; i++)
    {
        SignatureData empty;
        CHECK(SignSignature(keystore, CTransaction(txFrom), txTo[i], 0, SIGHASH_ALL, empty), strprintf("SignSignature %d", i));
        CHECK(IsStandardTx(CTransaction(txTo[i]), /*permit_bare_multisig=*/true, reason), strprintf("txTo[%d].IsStandard", i));
        bool no_pbms_is_std = IsStandardTx(CTransaction(txTo[i]), /*permit_bare_multisig=*/false, reason);
        CHECK((i == 0 ? no_pbms_is_std : !no_pbms_is_std), strprintf("txTo[%d].IsStandard(permbaremulti=false)", i));
    }
}

FIXTURE_TEST_CASE(is, BasicTestingSetup)
{
    // Test CScript::IsPayToScriptHash()
    uint160 dummy;
    CScript p2sh;
    p2sh << OP_HASH160 << ToByteVector(dummy) << OP_EQUAL;
    CHECK(p2sh.IsPayToScriptHash());

    std::vector<unsigned char> direct = {OP_HASH160, 20};
    direct.insert(direct.end(), 20, 0);
    direct.push_back(OP_EQUAL);
    CHECK(CScript(direct.begin(), direct.end()).IsPayToScriptHash());
    ...
    CHECK(!CScript(pushdata1.begin(), pushdata1.end()).IsPayToScriptHash());
    ...
    CHECK(!CScript(pushdata2.begin(), pushdata2.end()).IsPayToScriptHash());
    ...
    CHECK(!CScript(pushdata4.begin(), pushdata4.end()).IsPayToScriptHash());

    CScript not_p2sh;
    CHECK(!not_p2sh.IsPayToScriptHash());
...
FIXTURE_TEST_CASE(switchover, BasicTestingSetup)
{
    ...
    CHECK(Verify(scriptSig, fund, false, err));
    CHECK(err == SCRIPT_ERR_OK, ScriptErrorString(err));
    // Fail under new:
    CHECK(!Verify(scriptSig, fund, true, err));
    CHECK(err == SCRIPT_ERR_EQUALVERIFY, ScriptErrorString(err));
}

FIXTURE_TEST_CASE(ValidateInputsStandardness, BasicTestingSetup)
{
    ...
    CHECK(keystore.AddCScript(pay1));
    ...
    CHECK(keystore.AddCScript(oneAndTwo));
    ...
    SignatureData empty;
    CHECK(SignSignature(keystore, CTransaction(txFrom), txTo, 0, SIGHASH_ALL, empty));
    SignatureData empty_b;
    CHECK(SignSignature(keystore, CTransaction(txFrom), txTo, 1, SIGHASH_ALL, empty_b));
    SignatureData empty_c;
    CHECK(SignSignature(keystore, CTransaction(txFrom), txTo, 2, SIGHASH_ALL, empty_c));
    ...
    CHECK(GetP2SHSigOpCount(CTransaction(txTo), coins) == 22U);
    ...
    CHECK(GetP2SHSigOpCount(coinbase_tx, coins) == 0U);
    ...
    CHECK(txToNonStd1_res.GetRejectReason() == "bad-txns-nonstandard-inputs");
    CHECK(txToNonStd1_res.GetDebugMessage() == "p2sh redeemscript sigops exceed limit (input 0: 16 > 15)");

    CHECK(GetP2SHSigOpCount(CTransaction(txToNonStd1), coins) == 16U);
    ...
    CHECK(txToNonStd2_res.GetRejectReason() == "bad-txns-nonstandard-inputs");
    CHECK(txToNonStd2_res.GetDebugMessage() == "p2sh redeemscript sigops exceed limit (input 0: 20 > 15)");
    CHECK(GetP2SHSigOpCount(CTransaction(txToNonStd2), coins) == 20U);
    ...
    CHECK(txToNonStd2_no_scriptSig_res.GetRejectReason() == "bad-txns-nonstandard-inputs");
    CHECK(txToNonStd2_no_scriptSig_res.GetDebugMessage() == "input 0 P2SH redeemscript missing");
    CHECK(GetP2SHSigOpCount(CTransaction(txToNonStd2), coins) == 20U);
    ...
    CHECK(txToNonStd3_res.GetRejectReason() == "bad-txns-nonstandard-inputs");
    CHECK(txToNonStd3_res.GetDebugMessage() == "input 0 script unknown");
    ...
    CHECK(txToNonStd4_res.GetRejectReason() == "bad-txns-nonstandard-inputs");
    CHECK(txToNonStd4_res.GetDebugMessage() == "p2sh scriptsig malformed (input 0: OP_RETURN was encountered)");
    ...
    CHECK(txWitnessUnknown_res.GetRejectReason() == "bad-txns-nonstandard-inputs");
    CHECK(txWitnessUnknown_res.GetDebugMessage() == "input 0 witness program is undefined");
}
