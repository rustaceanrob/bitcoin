...
script_verify_flags ParseScriptFlags(std::string strFlags)
{
    ...
    for (const std::string& word : words)
    {
        if (!mapFlagNames.contains(word)) {
            RECORD_ERROR("Bad test: unknown verification flag '" << word << "'");
            continue;
        }
...
TEST_SUITE_BEGIN(transaction_tests)

FIXTURE_TEST_CASE(tx_valid, BasicTestingSetup)
{
    CHECK(CheckMapFlagNames(), "mapFlagNames is missing a script verification flag");
    ...
    for (unsigned int idx = 0; idx < tests.size(); idx++) {
        ...
        if (test[0].isArray())
        {
            ...
            if (~verify_flags != FillFlags(~verify_flags)) {
                RECORD_ERROR("Bad test flags: " << strTest);
            }

            CHECK(CheckTxScripts(tx, mapprevOutScriptPubKeys, mapprevOutValues, ~verify_flags, txdata, strTest, /*expect_valid=*/true), "Tx unexpectedly failed: " << strTest);
            ...
            for (const auto& [name, flag] : mapFlagNames) {
                ...
                if (!CheckTxScripts(tx, mapprevOutScriptPubKeys, mapprevOutValues, flags, txdata, strTest, /*expect_valid=*/true)) {
                    RECORD_ERROR("Tx unexpectedly failed with flag " << name << " unset: " << strTest);
                }
                // Removing random combinations of flags
                flags = TrimFlags(~(verify_flags | script_verify_flags::from_int(m_rng.randbits(MAX_SCRIPT_VERIFY_FLAGS_BITS))));
                if (!CheckTxScripts(tx, mapprevOutScriptPubKeys, mapprevOutValues, flags, txdata, strTest, /*expect_valid=*/true)) {
                    RECORD_ERROR("Tx unexpectedly failed with random flags " << ToString(flags.as_int()) << ": " << strTest);
                }
            }
            ...
            for (auto flags_excluding_one : ExcludeIndividualFlags(verify_flags)) {
                if (!CheckTxScripts(tx, mapprevOutScriptPubKeys, mapprevOutValues, ~flags_excluding_one, txdata, strTest, /*expect_valid=*/false)) {
                    RECORD_ERROR("Too many flags unset: " << strTest);
                }
            }
        }
    }
...
FIXTURE_TEST_CASE(tx_invalid, BasicTestingSetup)
{
    ...
    for (unsigned int idx = 0; idx < tests.size(); idx++) {
        ...
        if (test[0].isArray())
        {
            ...
            if (!CheckTransaction(tx, state) || state.IsInvalid()) {
                CHECK(test[2].get_str() == "BADTX", strTest);
                continue;
            }
            ...
            CHECK(CheckTxScripts(tx, mapprevOutScriptPubKeys, mapprevOutValues, verify_flags, txdata, strTest, /*expect_valid=*/false), "Tx unexpectedly passed: " << strTest);
            ...
            for (const auto& [name, flag] : mapFlagNames) {
                ...
                if (!CheckTxScripts(tx, mapprevOutScriptPubKeys, mapprevOutValues, flags, txdata, strTest, /*expect_valid=*/false)) {
                    RECORD_ERROR("Tx unexpectedly passed with flag " << name << " set: " << strTest);
                }
                // Adding random combinations of flags
                flags = FillFlags(verify_flags | script_verify_flags::from_int(m_rng.randbits(MAX_SCRIPT_VERIFY_FLAGS_BITS)));
                if (!CheckTxScripts(tx, mapprevOutScriptPubKeys, mapprevOutValues, flags, txdata, strTest, /*expect_valid=*/false)) {
                    RECORD_ERROR("Tx unexpectedly passed with random flags " << name << ": " << strTest);
                }
            }
            ...
            for (auto flags_excluding_one : ExcludeIndividualFlags(verify_flags)) {
                if (!CheckTxScripts(tx, mapprevOutScriptPubKeys, mapprevOutValues, flags_excluding_one, txdata, strTest, /*expect_valid=*/true)) {
                    RECORD_ERROR("Too many flags set: " << strTest);
                }
            }
        }
    }
...
FIXTURE_TEST_CASE(tx_no_inputs, BasicTestingSetup)
{
    CMutableTransaction empty;

    TxValidationState state;
    CHECK(!CheckTransaction(CTransaction(empty), state), "Transaction with no inputs should be invalid.");
    CHECK(state.GetRejectReason() == "bad-txns-vin-empty");
}

FIXTURE_TEST_CASE(tx_oversized, BasicTestingSetup)
{
    ...
    {
        TxValidationState state;
        CHECK(!CheckTransaction(createTransaction(maxPayloadSize), state), "Oversized transaction should be invalid");
        CHECK(state.GetRejectReason() == "bad-txns-oversize");
    }
}

FIXTURE_TEST_CASE(basic_transaction_tests, BasicTestingSetup)
{
    ...
    TxValidationState state;
    CHECK(CheckTransaction(CTransaction(tx), state), "Simple deserialized transaction should be valid.");
    CHECK(state.IsValid(), "Simple deserialized transaction should be valid.");

    // Check that duplicate txins fail
    tx.vin.push_back(tx.vin[0]);
    CHECK((!CheckTransaction(CTransaction(tx), state) || !state.IsValid()), "Transaction with duplicate txins should be invalid.");
}

FIXTURE_TEST_CASE(test_Get, BasicTestingSetup)
{
...
FIXTURE_TEST_CASE(test_big_witness_transaction, BasicTestingSetup)
{
...
FIXTURE_TEST_CASE(test_witness, BasicTestingSetup)
{
...
FIXTURE_TEST_CASE(test_IsStandard, BasicTestingSetup)
{
    FillableSigningProvider keystore;
    CCoinsViewCache coins{&CoinsViewEmpty::Get()};
    std::vector<CMutableTransaction> dummyTransactions =
    ...
    CHECK(83U == t.vout[0].scriptPubKey.size());
    ...
    CHECK(GetTransactionWeight(CTransaction(t)) == 400000);
    CheckIsStandard(t);

    // increase output size by one byte, so we end up with 400004 weight units
    t.vout[0].scriptPubKey = CScript() << OP_RETURN << std::vector<unsigned char>(20, 0); // output size: 31 bytes
    CHECK(GetTransactionWeight(CTransaction(t)) == 400004);
...
FIXTURE_TEST_CASE(max_standard_legacy_sigops, BasicTestingSetup)
{
    CCoinsViewCache coins{&CoinsViewEmpty::Get()};
    CKey key;
    key.MakeNewKey(true);
    ...
    CHECK(p2sh_inputs_count * MAX_P2SH_SIGOPS < MAX_TX_LEGACY_SIGOPS);
    AddCoins(coins, CTransaction(tx_create), 0, false);

    // 2490 sigops is below the limit.
    CHECK(GetP2SHSigOpCount(CTransaction(tx_max_sigops), coins) == 2490U);
    ...
    AddCoins(coins, CTransaction(tx_create), 0, false);
    CHECK((p2sh_inputs_count + 1) * MAX_P2SH_SIGOPS > MAX_TX_LEGACY_SIGOPS);
    auto legacy_sigops_count = GetP2SHSigOpCount(CTransaction(tx_max_sigops), coins);
    CHECK(legacy_sigops_count == 2505U);
    ...
    std::string sigop_limit_reject_debug_message("non-witness sigops exceed bip54 limit");
    {
        ...
        CHECK(validation_state.GetRejectReason() == reject_reason);
        CHECK(validation_state.GetDebugMessage() == sigop_limit_reject_debug_message);
    }
    ...
    CHECK(p2sh_inputs_count * MAX_P2SH_SIGOPS + p2pk_inputs_count * 1 == MAX_TX_LEGACY_SIGOPS);
    CHECK(::ValidateInputsStandardness(CTransaction(tx_max_sigops), coins).IsValid());
    ...
    REQUIRE(::ValidateInputsStandardness(CTransaction(tx_max_sigops), coins).IsValid());
    ...
    CHECK(legacy_sigop_count_p2pk > MAX_TX_LEGACY_SIGOPS);
    {
        auto validation_state = ValidateInputsStandardness(CTransaction(tx_max_sigops), coins);
        CHECK(validation_state.IsInvalid());
        CHECK(validation_state.GetRejectReason() == reject_reason);
...
FIXTURE_TEST_CASE(checktxinputs_invalid_transactions_test, BasicTestingSetup)
{
    auto check_invalid{[](CAmount input_value, CAmount output_value, bool coinbase, int spend_height, TxValidationResult expected_result, std::string_view expected_reason) {
        ...
        CHECK(state.GetResult() == expected_result);
        CHECK(state.GetRejectReason() == expected_reason);
    }};
...
FIXTURE_TEST_CASE(getvalueout_out_of_range_throws, BasicTestingSetup)
{
    CMutableTransaction mtx;
...
FIXTURE_TEST_CASE(spends_witness_prog, BasicTestingSetup)
{
    CCoinsViewCache coins{&CoinsViewEmpty::Get()};
    CKey key;
    key.MakeNewKey(true);
    ...
    CHECK(Solver(tx_create.vout[0].scriptPubKey, sol_dummy) == TxoutType::PUBKEY);
    ...
    CHECK(Solver(tx_create.vout[0].scriptPubKey, sol_dummy) == TxoutType::PUBKEYHASH);
    tx_spend.vin[0].prevout.hash = tx_create.GetHash();
    AddCoins(coins, CTransaction{tx_create}, 0, false);
    CHECK(!::SpendsNonAnchorWitnessProg(CTransaction{tx_spend}, coins));
    ...
    CHECK(Solver(tx_create.vout[0].scriptPubKey, sol_dummy) == TxoutType::SCRIPTHASH);
    tx_spend.vin[0].prevout.hash = tx_create.GetHash();
    ...
    CHECK(Solver(tx_create.vout[0].scriptPubKey, sol_dummy) == TxoutType::WITNESS_V0_SCRIPTHASH);
    tx_spend.vin[0].prevout.hash = tx_create.GetHash();
    AddCoins(coins, CTransaction{tx_create}, 0, false);
    CHECK(::SpendsNonAnchorWitnessProg(CTransaction{tx_spend}, coins));
    ...
    CHECK(Solver(tx_create.vout[0].scriptPubKey, sol_dummy) == TxoutType::WITNESS_V0_KEYHASH);
    tx_spend.vin[0].prevout.hash = tx_create.GetHash();
    AddCoins(coins, CTransaction{tx_create}, 0, false);
    CHECK(::SpendsNonAnchorWitnessProg(CTransaction{tx_spend}, coins));
    ...
    CHECK(Solver(tx_create.vout[0].scriptPubKey, sol_dummy) == TxoutType::WITNESS_V1_TAPROOT);
    tx_spend.vin[0].prevout.hash = tx_create.GetHash();
    AddCoins(coins, CTransaction{tx_create}, 0, false);
    CHECK(::SpendsNonAnchorWitnessProg(CTransaction{tx_spend}, coins));
    ...
    CHECK(Solver(tx_create.vout[0].scriptPubKey, sol_dummy) == TxoutType::ANCHOR);
    tx_spend.vin[0].prevout.hash = tx_create.GetHash();
    AddCoins(coins, CTransaction{tx_create}, 0, false);
    CHECK(!::SpendsNonAnchorWitnessProg(CTransaction{tx_spend}, coins));
    ...
    CHECK(Solver(tx_create.vout[0].scriptPubKey, sol_dummy) == TxoutType::WITNESS_UNKNOWN);
    tx_spend.vin[0].prevout.hash = tx_create.GetHash();
    AddCoins(coins, CTransaction{tx_create}, 0, false);
    CHECK(::SpendsNonAnchorWitnessProg(CTransaction{tx_spend}, coins));
