...
TEST_SUITE_BEGIN(pool_tests)

FIXTURE_TEST_CASE(basic_allocating, BasicTestingSetup)
{
    ...
    CHECK(expected_bytes_available == PoolResourceTester::AvailableMemoryFromChunk(resource));
    ...
    CHECK(0U == PoolResourceTester::FreeListSizes(resource)[1]);
    resource.Deallocate(block, 8, 8);
    PoolResourceTester::CheckAllDataAccountedFor(resource);
    CHECK(1U == PoolResourceTester::FreeListSizes(resource)[1]);

    // alignment is too small, but the best fitting freelist is used. Nothing is allocated.
    void* b = resource.Allocate(8, 1);
    CHECK(b == block); // we got the same block of memory as before
    CHECK(0U == PoolResourceTester::FreeListSizes(resource)[1]);
    CHECK(expected_bytes_available == PoolResourceTester::AvailableMemoryFromChunk(resource));
...
FIXTURE_TEST_CASE(allocate_any_byte, BasicTestingSetup)
{
...
FIXTURE_TEST_CASE(random_allocations, BasicTestingSetup)
{
    ...
    for (size_t i = 0; i < 1000; ++i) {
        ...
        if (ptr_size_alignment.empty() || 0 != m_rng.randrange(4)) {
            ...
            CHECK(ptr != nullptr);
            CHECK((reinterpret_cast<uintptr_t>(ptr) & (alignment - 1)) == 0U);
...
FIXTURE_TEST_CASE(memusage_test, BasicTestingSetup)
{
...

    {
        ...
        CHECK(memusage::DynamicUsage(std_map) != memusage::DynamicUsage(resource_map));
        ...
        CHECK(memusage::DynamicUsage(resource_map) <= memusage::DynamicUsage(std_map) * 90 / 100);
        ...
        CHECK(resource.NumAllocatedChunks() >= min_num_allocated_chunks);
    }
