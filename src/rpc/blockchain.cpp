...
static RPCHelpMan genhints()
{
    return RPCHelpMan{
        ...
        RPCResult{
            RPCResult::Type::OBJ, "", "",
                {
                    {RPCResult::Type::STR, "height", "The stopping height encoded by the hint file."},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
        ...
        for (const auto& tx: pblock->vtx) {
            ...
            for (size_t vout = 0; vout < tx->vout.size(); vout++) {
                ...
                if (active_state.CoinsDB().HaveCoin(outpoint)) {
                    unspent_offsets.emplace_back(curr_offset);
                    curr_offset = 0;
                }
                curr_offset++;
            }
        }
        if (!writer.WriteNextUnspents(unspent_offsets, (uint32_t)height)) {
    ...
    result.pushKV("path", path.utf8string());
    result.pushKV("duration", std::to_string(duration) + " minutes.");
    return result;
},
    };
}
